"""522 ファイル全部を per-file ML overfit で処理し、binary match 数を集計。

各ファイル:
  1. per-file v8 CSV 生成 (Rust binary 呼び出し)
  2. LightGBM 過学習 (1500 rounds, num_leaves=4096)
  3. encode + binary 比較
  4. 結果を CSV に追記

進捗保存: /tmp/lf2_ml_batch_results.csv
チェックポイント機能あり (途中再開可能)
"""
...
RESULTS_CSV = Path("/tmp/lf2_ml_batch_results.csv")
RUST_BIN = Path("/home/ariori/repos/2025/retro-decode/target/release/lf2_pairwise_dataset_v8")
...
def process_one(lf2_path, num_rounds=1500):
    name = lf2_path.stem
    csv_path = WORK_DIR / f"{name}_v8.csv"
    ...
    try:
        ...
        return {"file": name, "status": f"csv_load_failed: {e}", "match": 0}

    if len(df) == 0:
        # File has no multi-pos tokens (all forced) → trivially matches if leaf decode works
        lf2_bytes = lf2_path.read_bytes()
        leaf_tokens, payload_start = decode_full_tokens(lf2_bytes)
        new_payload = frame_payload(leaf_tokens)
        match = 1 if new_payload == lf2_bytes[payload_start:] else 0
        return {"file": name, "status": "no_multi_tokens", "match": match,
                "tokens": len(leaf_tokens), "multi": 0}
    ...
    try:
        ...
        return {"file": name, "status": f"train_failed: {e}", "match": 0}
...
def main():
    files = sorted(LF2_DIR.glob("*.LF2"))
    print(f"found {len(files)} LF2 files")
    ...
    with open(RESULTS_CSV, "a") as f:
        writer = csv.DictWriter(f, fieldnames=["file", "status", "match", "tokens", "multi", "elapsed_s"])
        if write_header:
            writer.writeheader()

        total_match = sum(1 for _ in range(0))  # placeholder
        for i, p in enumerate(files):
            ...
            try:
                ...
                r = {"file": p.stem, "status": f"exception: {e}", "match": 0, "tokens": 0, "multi": 0}
            r["elapsed_s"] = round(time.time() - t, 1)
            writer.writerow(r)
            f.flush()
            print(f"[{i+1}/{len(files)}] {r['file']}: match={r['match']} status={r['status']} ({r['elapsed_s']}s)", flush=True)
