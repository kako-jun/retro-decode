...
### Stage 10: KIND_DIFF の再分類とダミー領域仮説 (Issue #14、2026-07-18)

**Stage 10 (再分類)**: Stage 9 系の KIND_DIFF (トークン種別自体が Leaf/Sim で
相違) を、Stage 9-2d の per-file 勝ちモード (Clip が byte-exact ならそれ、
でなければ Plus1) を Sim として再集計。旧 rank1_minage ベースの 57 本から
**49 本**に整理 (`lf2_stage10_debug`)。**49/49 全件が Leaf=Literal / Sim=Match**
のパターン (逆パターンは 0 件)。divergence 点は tail 近傍ではなく、ファイル
中盤〜広範囲に分布 (Stage 9 系とは別種の divergence と確定)。

**Stage 10-2 (出現解析)**: 49 本の divergence 点で Sim が選んだ match 文字列の
リング上の全出現を brute-force 列挙。**49 本中 42 本 (86%) で、Sim の match
position が「一度も実際に書き込まれていない」リング初期化領域 (0x20 埋め、
`write_tick` 未設定) を指している**と判明 (`lf2_stage10_2_debug`)。

**Stage 10-3 (no-dummy 仮説の検証・棄却)**: 「Leaf は未書込み領域へのマッチを
許可しない」仮説を `DummyMode::RejectAny` / `RejectAllUnwritten` として実装し
522 本フル計測。**明確に反証**: Clip 165→2、Plus1 167→1 (RejectAny) / 165→4、
167→3 (RejectAllUnwritten) と壊滅的に一致数が崩壊し、fixed は 0 本、union も
197 のまま変化なし。Leaf は未書込み領域へのマッチを実際には**日常的に大量に
使っている** (165 本中 163 本がこれに依存)。

**Stage 10-4 (拒否候補の局在発見)**: KIND_DIFF ダミー系 42 本全件で、Sim の
拒否候補 (`rej_pos`) が **例外なく `[4060, 4077]` の 18 スロット幅**に collapse
すると判明。これは奥村実装のブートストラップ処理
`for i in 1..=F { insert_node(r - i) }` (`r = N - F = 4078`) が挿入する
**合成ダミーノードの位置そのもの** (`r-1..r-F`)。対して Leaf 自身が採用した
「未書込みを含む match」2197 件のうち、窓が完全未書込みなのは 11 件 (0.5%)
のみで、その最大長は 10・位置は帯の中央寄り4点 (4068/4069/4070/4075) に限られ、
帯の端 (4076/4077、最新のダミーノード) は皆無だった。

**write_tick 偽陽性ギャップ (要注意)**: 初期 F バイト先読み充填
(`text_buf[r+len]=input[input_idx]`、範囲 `r..r+F-1` = `4078..4095`) は
`write_tick` を更新しない実装になっているため、この範囲は実データが書かれて
いても「未書込み」と誤分類される (Stage 10-4 の `contradiction_found` 全件
true はこの偽陽性が主因)。ただしブートストラップダミーノード帯
`[4060,4077]` は `r=4078` **未満**であり先読み充填の範囲に含まれないため、
本 Stage (10-3〜10-5) の結論・variant (v1/v2/v3) には影響しない。

**Stage 10-5 (predicate variant 実測)**: 上記の局在に基づく 3 variant を
{Clip, Plus1} でクロス計測 (522 本フル):

| variant | 条件 | Clip | Plus1 |
|---|---|---|---|
| v1 | 帯 `[4060,4077]` かつ窓が全域未書込みなら不採用 | 163/522 | 152/522 |
| v2 | v1 + `len>10` のみ不採用 | 164/522 | 152/522 |
| v3 | 帯端 (4076/4077) のみ不採用 (窓条件なし) | 18/522 | 16/522 |

v3 は壊滅 (18/16) で棄却。v1/v2 は単体では退行するが (Plus1 で最大 21 本
喪失)、**8 モード全体の union は 197→203 に純増 (+6)**: `H53, H55, S34D,
S42D, S43D, V4F` の 6 本 (いずれも KIND_DIFF ダミー系 42 本のメンバー) を
v1 が回収。KIND_DIFF 42 本中 36 本は依然未解決。

回収できた 6 本 (C1201/H51 等の代替サンプル観測) では、Sim の拒否候補を
弾いた後 Leaf は「全く別の実書込み位置にある同じ長さの match」に切り替えて
いた。これは「候補を1つ弾けば次善の match に自動的に切り替わる」という
単純なモデルではなく、**lazy matching (1 バイト先送りして次の match 候補を
覗く) に類する探索順序の違い**を示唆する。次手はこの lazy matching 仮説の
直接検証 (1 バイト遅延した窓での候補比較)。
