//! Pairwise dataset v3: v2 + 履歴トークン特徴量。
//!
//! v2 (lf2_pairwise_dataset_v2.rs) からの追加特徴量:
//! - token-level (全候補で同じ値): prev_2_kind, prev_2_len, prev_3_kind, prev_3_len
//!
//! 目的: セッション 301 で lambdarank +2.2pt は得られたが ≥90% 帯は逆に減った。
//! ファイル内決定論性が 100% 確認されたため、ファイル内の「過去 N トークンの履歴」
//! が選択の決定要因である可能性が高い。ring 状態とは別軸の情報として履歴トークンを追加。
...
fn main() -> ExitCode {
    ...
    if args.len() < 2 {
        eprintln!("usage: lf2_pairwise_dataset_v3 <input_dir_or_file> [output_csv]");
    ...
    let out_path = args
        .get(2)
        .cloned()
        .unwrap_or_else(|| "/tmp/lf2_pairwise_v3.csv".to_string());
    ...
    if let Err(e) = writeln!(
        writer,
        "file,token_idx,ring_r,n_cands,n_max,max_len,cand_pos,cand_len,cand_dist,is_leaf,prev_kind,prev_len,img_w,img_h,img_colors,in_byte,in_byte_p1,in_byte_p2,in_byte_after,r_bef1,r_bef2,r_aft,r_aft1,prev_2_kind,prev_2_len,prev_3_kind,prev_3_len"
    ) {
