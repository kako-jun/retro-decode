//! Pairwise dataset v9: v8 + リング周回数 + 画像サイズ派生 + 交互作用項。
//!
//! 仮説:
//! - 仮説 A (リング周回): C1001 (50 tokens, リング 0 周, 100%) と OC_07 (3000+ tokens, 99.29%) の差は
//!   「リング N 周目以降に発動する if」が原因。OC_07 の 23 miss は全て input_pos 7508-11365
//!   (= リング複数周後)。1 周目までは greedy + 縦アラインメントで足りるが、2 周目以降に別 if が発動。
//! - 仮説 B (画像サイズ依存): img_w 単独の重要度は rank 6 だが派生がない。
//!   total_pixels / colors_bucket / aspect_ratio で「LF2 ヘッダ依存の冒頭分岐」を捕捉。
//! - 仮説 C (交互作用): 決定木は単独特徴量から非線形を学べるが明示的交互作用は学習が遅い。
//!   wrap_count × dist / wrap_count × dist_mod_w を直接渡してブースト初期段階で利かせる。
//!
//! v8 (lf2_pairwise_dataset_v8.rs) からの追加特徴量:
//! - token-level (5 cols): wrap_count, wrap_phase, total_pixels, colors_bucket, aspect_ratio
//! - candidate-level (2 cols): wrap_x_dist, wrap_x_mod_w
//!
//! CSV 53→60 列。
//! 観察ポイント: wrap_count が importance top 10 入り / CWEEK_02・CSNO_07 の 100% 到達 /
//!              OC_07 が 99.5% を越えるか / 完全一致 1→3+。
...
fn main() -> ExitCode {
    ...
    if args.len() < 2 {
        eprintln!("usage: lf2_pairwise_dataset_v9 <input_dir_or_file> [output_csv]");
    ...
    let out_path = args
        .get(2)
        .cloned()
        .unwrap_or_else(|| "/tmp/lf2_pairwise_v9.csv".to_string());
