//! Pairwise dataset v8: v7 + 画像縦方向アラインメント特徴量。
//!
//! v7 + CWEEK_02 唯一の miss (token 327) の解析で発見:
//!   候補 A (dist=97) と候補 B (dist=177=97+img_w) のうち Leaf は B を選択。
//!   つまり Leaf は「真上の行の同列」(dist が img_w の倍数) を強く優先する。
//!   col_pos/row_pos はトークン位置のみで候補ごとの差を表現できなかった。
//!
//! v7 (lf2_pairwise_dataset_v7.rs) からの追加特徴量:
//! - candidate-level: cand_dist_mod_w (cand_dist % img_w), cand_dist_div_w (cand_dist / img_w)
//!
//! cand_dist_mod_w == 0 のときは「真上の行の同じ列」、近い値のときは「斜めにずれた同行」。
//! Leaf の縦方向アラインメント仕様を直接特徴量化。CSV 51→53 列。
//! 期待: CWEEK_02 → 100% 達成、OC 系列の同種 miss 回収。
            ...
            if n_max > N_MAX_CAP {
                ...
                for c in candidates.iter().filter(|c| c.len == max_len) {
                    ...
                    writeln!(
                        ...
                        // v8 new (2 cols, image-aligned distance)
...
fn main() -> ExitCode {
    ...
    if args.len() < 2 {
        eprintln!("usage: lf2_pairwise_dataset_v8 <input_dir_or_file> [output_csv]");
    ...
    let out_path = args
        .get(2)
        .cloned()
        .unwrap_or_else(|| "/tmp/lf2_pairwise_v8.csv".to_string());
