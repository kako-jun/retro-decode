//! Stage 12-11 (Issue #14 脈: 「腐った木」仮説) 既存ゲート再計測:
//! binary tie 50件 / α7 / C120x token3-4 を RotA/RotB で確認する。
//!
//! usage:
//!   cargo run --release --bin lf2_stage12_11_gate -- <DIR>
...
const VARIANTS: [(SimMode, &str); 3] = [
    (SimMode::Basic, "Basic(対照)"),
    (SimMode::RotANoDelete, "RotA(no-delete)"),
    (SimMode::RotBNoDeleteNoReplace, "RotB(no-delete+no-replace)"),
];
...
fn main() -> ExitCode {
    ...
    eprintln!("=== Gate (a): binary tie {} events ===", events.len());
    ...
    for &(mode, label) in &VARIANTS {
        ...
        eprintln!("  {:30}: ti=3 hit {}/{}  ti=4 hit {}/{}", label, any_hit_ti3, C120X.len(), any_hit_ti4, C120X.len());
    }
