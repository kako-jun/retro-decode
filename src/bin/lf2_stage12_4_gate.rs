//! Stage 12-4 (Issue #14 脈3 再解釈): 「挿入タイミング=書込み時」BST 亜種の
//! 安価なゲート検証 (522本フル計測の前段)。tie-break は原典 First (`>`) 固定、
//! 木構造 (挿入タイミング/順序) だけを変える。実装変更なし・観測専用
//! (新規 SimMode バリアント自体は okumura_lzss.rs に追加済み)。
//!
//! ゲート:
//!   (a) binary tie 50件 (n_candidates==2): 新しい木で標準 First 探索が
//!       winner (Leaf 実採用位置) を当てる数。目標 45/50。
//!   (b) α群7本: 見逃し候補 (旧 Sim が採用したが Leaf が使わなかった実書込み
//!       候補) が新しい木で「不在」または「経路外」になるか。目標 7/7。
//!   (c) C120x系 token 3-4: 位置 4092/4093 が木に候補として現れるか。
//!
//! usage:
//!   cargo run --release --bin lf2_stage12_4_gate -- <DIR>
...
const VARIANTS: [(SimMode, &str); 5] = [
    ...
    (SimMode::WriteTimeAscending, "WriteTimeAscending"),
    (SimMode::WriteTimeDescending, "WriteTimeDescending"),
    (SimMode::WriteTimeAscendingKeepDummy, "WriteTimeAscending+KeepDummy"),
    (SimMode::WriteTimeDescendingKeepDummy, "WriteTimeDescending+KeepDummy"),
];
...
/// C120x 系: token index 4 (0-indexed di=4、「token 3〜4」の記述に合わせ両方調べる) で
/// 4092/4093 が候補に見えるかを確認する対象ファイル群。
const C120X: [&str; 14] = [
...
fn main() -> ExitCode {
    ...
    for &(mode, label) in &VARIANTS {
        let mut resolved = 0usize; // 不在 or 経路外 の件数
        eprintln!("  --- {} ---", label);
