//! Pairwise dataset v12: v8 + BST 完全状態シミュレーション特徴量 (Issue #14)。
//!
//! Leaf の実トークン列で 4 モードの奥村 BST (`OkumuraSim`) を teacher-forcing
//! 進行させ、各 tie token 直前に read-only トレース (`search_trace`) を実行、
//! 候補ごとに「探索が何番目に訪問するか (rank)」「木の深さ (depth)」を記録する。
//! cross-tie 衝突 (同一ローカル特徴で違う選択) が BST の完全状態で分離される
//! かを検証するのが目的。
//!
//! v8 (lf2_pairwise_dataset_v8.rs) からの追加カラム (末尾 8 列, 53→61 列):
//! - bst_rank_basic / bst_rank_nodummy / bst_rank_dtd / bst_rank_leftfirst (u32)
//!   1 = 探索が最初に見つける max_len ノード。0 = 候補が木に不在
//! - bst_depth_basic / bst_depth_nodummy / bst_depth_dtd / bst_depth_leftfirst (u8)
//!   root からの段数。不在 = 255
//!
//! usage: cargo run --release --bin lf2_pairwise_dataset_v12 -- <LF2ディレクトリ> <出力CSV>
...
/// search_trace の結果から候補 pos の (rank, depth) を引く。
/// 不在なら rank=0, depth=255。
#[inline]
fn trace_lookup(trace: &[(u16, u32, u8)], pos: u16) -> (u32, u8) {
    trace
        .iter()
        .find(|t| t.0 == pos)
        .map(|t| (t.1, t.2))
...
fn process_file(
    ...
    // v12: 4 モードの BST 完全状態シミュレータ (teacher forcing)
    let mut sims = [
    ...
    for (token_idx, tok) in leaf.iter().enumerate() {
        ...
        if leaf_kind == 'M' && n_max >= 2 && max_len >= 3 {
            if n_max > N_MAX_CAP {
                ...
                let traces: [Vec<(u16, u32, u8)>; 4] = [
                    ...
                    sims[2].search_trace(sims[2].r, max_len),
                    sims[3].search_trace(sims[3].r, max_len),
                ];

                for c in candidates.iter().filter(|c| c.len == max_len) {
                    let dist = (r + 0x1000 - c.pos as usize) & 0x0fff;
                    let is_leaf = (leaf_pos as u16 == c.pos && leaf_len_v as u8 == c.len) as u8;
                    ...
                    let (bst_rank_nodummy, bst_depth_nodummy) = trace_lookup(&traces[1], c.pos);
                    let (bst_rank_dtd, bst_depth_dtd) = trace_lookup(&traces[2], c.pos);
                    let (bst_rank_leftfirst, bst_depth_leftfirst) = trace_lookup(&traces[3], c.pos);
                    ...
                    writeln!(
                        ...
                        // v12 new (8 cols, BST full-state simulation)
                        bst_rank_basic, bst_rank_nodummy, bst_rank_dtd, bst_rank_leftfirst,
                        bst_depth_basic, bst_depth_nodummy, bst_depth_dtd, bst_depth_leftfirst,
                    )?;
                    rows += 1;
                }
            }
        ...
        // v12: tie 判定に関係なく全 token で 4 sim を Leaf 実出力バイトで進める
        let emit_end = (input_pos + l).min(input.len());
        let emitted = &input[input_pos..emit_end];
        ...
        // v12: sim の r と ring ループの r が全 token で一致することを常時検証
...
fn main() -> ExitCode {
    ...
    if args.len() < 2 {
        eprintln!("usage: lf2_pairwise_dataset_v12 <input_dir_or_file> [output_csv]");
    ...
    let out_path = args
        .get(2)
        .cloned()
        .unwrap_or_else(|| "/tmp/lf2_pairwise_v12.csv".to_string());
