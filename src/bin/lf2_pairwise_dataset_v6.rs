//! Pairwise dataset v6: v5 + 候補書込み age (BST 順序の代理特徴量)。
//!
//! v5 (lf2_pairwise_dataset_v5.rs) からの追加特徴量:
//! - candidate-level: cand_age_start, cand_age_end (2 バイト相当、u32→f32)
//!   各候補位置 c.pos と c.pos + max_len - 1 の最終書込み時の input_pos との差分。
//!   未書込み (init slot) は 0xFFFFFFFF を採用 → モデルに「初期 ring」が伝わる。
//!
//! 目的: セッション 316 v5 BIG で完全一致 0→1 達成、≥95% 59→69。残る 99% 壁の OC_07 等は
//! cand_dist だけでは捕捉できない決定要因がある。BST 由来の "新鮮さ"(最近書き込まれた候補)
//! を直接特徴量化して、rank=2 押し負け 54% を回収する。CSV 45→47 列。
...
fn main() -> ExitCode {
    ...
    if args.len() < 2 {
        eprintln!("usage: lf2_pairwise_dataset_v6 <input_dir_or_file> [output_csv]");
    ...
    let out_path = args
        .get(2)
        .cloned()
        .unwrap_or_else(|| "/tmp/lf2_pairwise_v6.csv".to_string());
