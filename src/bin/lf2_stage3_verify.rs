//! Stage 3 (Issue #14): Stage 2 確定の複合タイブレイク規則エンコーダ
//! (`compress_okumura_rank1_minage`) の byte-exact 一致数を実測する。
//!
//! 各 LF2 ファイルについて:
...
//!
//! usage:
//!   cargo run --release --bin lf2_stage3_verify -- <LF2_DIR> [--limit N] [--out MATCHED_TXT]
//!
//! 出力:
...
use retro_decode::formats::toheart::okumura_lzss::{compress_okumura_rank1_minage, Token};
...
fn main() -> ExitCode {
    ...
    for path in &files {
        ...
        let tokens = compress_okumura_rank1_minage(&decoded.ring_input);
