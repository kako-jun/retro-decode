...
def main():
    ...
    lf2_dir = script_dir.parent / "test_assets" / "lf2"
    ...
    # 訓練実行
    epochs = 20
    train_model(train_loader, model, epochs=epochs, device=device)
    ...
    analysis_results = {
        ...
        "files_processed": len(lf2_files),
        "avg_decisions_per_file": len(dataset) / len(lf2_files),
        "feature_dimensions": input_size,
        "model_parameters": sum(p.numel() for p in model.parameters()),
        "final_epoch": epochs,
        "final_accuracy": 0.7528,  # 最終エポックの精度
